// Primitive Data Types
// Strings: Textual data enclosed "" or ''
let firstName = "John";
console.log("First Name:", firstName);
// typeof is an operator which identifies the data type of a variable/literal
console.log("the data type of firstName is:", typeof firstName);
// Number: Represents a numeric value (30,-30,3.14)
let age = 30;
console.log(age);
console.log("the data type of age is:", typeof age);
// Boolean: either true or false
let isAdmin = true;
console.log("Is admin:", isAdmin);
console.log("the data type of isAdmin is:", typeof isAdmin);

// Boolean: either true or false
let isAdmin = true;
console.log("Is admin:", isAdmin);
console.log("the data type of isAdmin is:", typeof isAdmin);
// Null: Intentional absence of a value
let car = null;
console.log("Car:", car);
console.log("the data type of car is:", typeof car);
// Undefined: Unintentional absence of a value
let city;
console.log("City:", city);
